    ...
    public Map<String, Long> statsPorResultado(String documento,
        ...
        return filtrados.stream()
                .collect(Collectors.groupingBy(
                        log -> Optional.ofNullable(log.getResultadoRisco()).orElse("DESCONHECIDO"),
                        Collectors.counting()
                ));
    }
...
");
    ...
    private List<LogAntifraude> filtrar(List<LogAntifraude> base,
        ...
        return base.stream()
                .filter(log -> documento == null || (log.getDocumentoUsuario() != null && log.getDocumentoUsuario().equals(documento)))
                .filter(log -> chaveDestino == null || (log.getChaveDestino() != null && log.getChaveDestino().equals(chaveDestino)))
                .filter(log -> resultadoRisco == null || (log.getResultadoRisco() != null && log.getResultadoRisco().equalsIgnoreCase(resultadoRisco)))
                .filter(log -> {
